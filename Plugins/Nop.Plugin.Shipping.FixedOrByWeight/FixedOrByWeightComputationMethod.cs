...
namespace Nop.Plugin.Shipping.FixedOrByWeight
{
    ...
    public class FixedOrByWeightComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        private readonly IWorkContext _workContext = EngineContext.Current.Resolve<IWorkContext>();//edit here by amal
            ...
            int storeId, int warehouseId, int countryId, int stateProvinceId, string zip)
        {
            ...
            var city = _workContext.CurrentCustomer.ShippingAddress.Country.Id; 
            var cart = EngineContext.Current.Resolve<IWorkContext>().CurrentCustomer.ShoppingCartItems
            .Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart)
            .Where(sci => sci.StoreId == EngineContext.Current.Resolve<IStoreContext>().CurrentStore.Id)
            .ToList();

            var normalproductweight = decimal.Zero;
            var shippingTotal = decimal.Zero;
            var shippingTotalcat = decimal.Zero;
            var shippingByWeightRecordDefault = _shippingByWeightService.FindRecord(0, shippingMethodId, storeId, warehouseId, countryId, stateProvinceId, zip, weight);
            var shippingByWeightRecordDefaultfee = decimal.Zero;
            var checklocation = false;
            if (countryId == city)
            {
                foreach (var c in cart)
                {
                    var productcat = c.Product.ProductCategories;
                        shippingTotalcat = decimal.Zero;
                    foreach (var pc in productcat)
                    {
                        var shippingByWeightRecord = _shippingByWeightService.FindRecord(pc.CategoryId, shippingMethodId, storeId, warehouseId, countryId, stateProvinceId, zip, weight);
                        if (shippingByWeightRecord != null && shippingByWeightRecord.CategoryId != 0)
                        {
                            shippingTotalcat += (shippingByWeightRecord.AdditionalFixedCost*c.Quantity);
                        }
                    }
                    if (shippingTotalcat == 0)
                    {
                        checklocation = true;
                        normalproductweight += (c.Product.Weight * c.Quantity);                 
                    }
                    else
                    {
                        shippingTotal += shippingTotalcat;
                    }  
                }
                if (shippingByWeightRecordDefault != null && checklocation)
                {
                    shippingByWeightRecordDefaultfee = shippingByWeightRecordDefault.AdditionalFixedCost;
                }

                if (normalproductweight > 3)
                {
                    shippingTotal = shippingTotal + 3 * (normalproductweight - 3) + shippingByWeightRecordDefaultfee;
                }
                else
                {
                    shippingTotal = shippingTotal + shippingByWeightRecordDefaultfee;
        ...
        public override void Install()
        {
            ...
            //amal 25/10/2018
            this.AddOrUpdatePluginLocaleResource("Plugins.Shipping.FixedOrByWeight.Fields.Category", "Category");
            this.AddOrUpdatePluginLocaleResource("Plugins.Shipping.FixedOrByWeight.Fields.Category.Hint", "If an asterisk is selected, then this shipping rate will apply to all customers, regardless of the Category.");
        ...
        public override void Uninstall()
        {
            ...
            //amal 25/10/2018
            this.DeletePluginLocaleResource("Plugins.Shipping.FixedOrByWeight.Fields.Category");
            this.DeletePluginLocaleResource("Plugins.Shipping.FixedOrByWeight.Fields.Category.Hint");
