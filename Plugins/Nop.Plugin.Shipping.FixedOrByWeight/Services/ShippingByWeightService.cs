...
namespace Nop.Plugin.Shipping.FixedOrByWeight.Services
{
    ...
    public partial class ShippingByWeightService : IShippingByWeightService
    {
        ...
        public virtual ShippingByWeightRecord FindRecord(int categoryId, int shippingMethodId, int storeId, int warehouseId, 
            int countryId, int stateProvinceId, string zip, decimal weight)
        {
            ...
            //filter by category
            var matchedByCategory = categoryId == 0
                ? matchedByCountry
                : matchedByCountry.Where(r => r.CategoryId == categoryId || r.CategoryId == 0);

            //filter by state/province
            var matchedByStateProvince = stateProvinceId == 0
                ? matchedByCategory
                : matchedByCategory.Where(r => r.StateProvinceId == stateProvinceId || r.StateProvinceId == 0);
           ...
           var foundRecords = matchedByZip.OrderBy(r => r.StoreId == 0).ThenBy(r => r.WarehouseId == 0)
              .ThenBy(r => r.CountryId == 0).ThenBy(r => r.CategoryId == 0).ThenBy(r => r.StateProvinceId == 0)
              .ThenBy(r => string.IsNullOrEmpty(r.Zip));

            return foundRecords.FirstOrDefault(x => x.CategoryId==categoryId && x.CountryId == countryId);
        }
