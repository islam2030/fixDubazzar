...
namespace Nop.Web.Infrastructure
{
    ...
    public partial class RouteProvider : IRouteProvider
    {
        ...
        public void RegisterRoutes(IRouteBuilder routeBuilder)
        {
            ...
            //cancel order for customer  11-6-2018
            routeBuilder.MapLocalizedRoute("CancelOrder", "cancelorder/{orderId}",
                            new { controller = "Order", action = "Cancel" },
                            new { orderId = @"\d+" },
                            new[] { "Nop.Web.Controllers" });

            //Bucket
            routeBuilder.MapRoute("Create", "Bucket/Create",
                    new { controller = "Bucket", action = "Create" });
            routeBuilder.MapRoute("Index", "Bucket/MyBuckets",
                    new { controller = "Bucket", action = "Index" });
            routeBuilder.MapRoute("Details", "Bucket/Details/{id}",
                new { controller = "Bucket", action = "Details" });
            //routeBuilder.MapRoute("Edit", "Bucket/Edit/{id}",
            //  new { controller = "Bucket", action = "Edit" });
            //routeBuilder.MapRoute("Delete", "Bucket/Delete/{id}",
            //  new { controller = "Bucket", action = "Delete" });
            //Contribution
           routeBuilder.MapRoute("SendGift", "Gift/Send/{BucketCode}",
           new { controller = "Contribution", action = "Create" });
            routeBuilder.MapRoute("BucketDetailsForFriends", "Gift/request/{BucketCode}",
      new { controller = "Contribution", action = "Details" });
        }
